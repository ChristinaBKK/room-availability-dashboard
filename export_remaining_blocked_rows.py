#!/usr/bin/env python3
"""Export only the remaining blocked rows from the latest column J retry results."""
...
SOURCE_CSV = Path("candidate_room_still_actionable_results.csv")
OUT_CSV = Path("candidate_room_remaining_blocked.csv")
OUT_XLSX = Path("candidate_room_remaining_blocked.xlsx")
TARGET_STATUS = "Still blocked"
...
def filter_rows(rows: list[dict[str, str]]) -> list[dict[str, str]]:
    return [row for row in rows if row.get("action_status") == TARGET_STATUS]
...
def write_xlsx(rows: list[dict[str, str]]) -> None:
    ...
    with ZipFile(OUT_XLSX, "w", compression=ZIP_DEFLATED) as zf:
        ...
        zf.writestr(
            ...
            '<dc:title>Candidate Room Remaining Blocked</dc:title>'
        ...
        zf.writestr(
            ...
            '<sheets><sheet name="Remaining Blocked" sheetId="1" r:id="rId1"/></sheets></workbook>',
        )
        zf.writestr(
...
def main() -> int:
    ...
    print(OUT_CSV)
    print(OUT_XLSX)
    print({"rows": len(rows), "status": TARGET_STATUS})
    return 0
